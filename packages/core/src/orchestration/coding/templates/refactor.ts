...
export function buildRefactorTemplate(params: RefactorParams): WorkflowNode[] {
  ...
  const analyst: WorkflowNode = {
    ...
    agent: {
      ...
      task: `Analyze the file dependency graph to identify safe parallel refactoring boundaries.\n\nRefactoring task: ${task}\n\nOutput a FanOutDecision JSON (same schema as feature-implementation architect) that:\n1. Groups files into independent clusters (no cross-cluster imports that would create merge conflicts)\n2. Identifies shared files that need stitcher coordination\n3. Creates specific refactoring instructions per chunk\n\n**Multi-phase spec override (Task #174):** if the user task references a specification document (\`*.md\` / \`*.txt\` / \`*.spec\`) that contains 3 or more \`## Phase N\` / \`### Phase N\` / \`## Step N\` / numbered top-level headings, you MUST emit one chunk per phase (e.g. 6 chunks for a 6-phase spec) instead of the default 2–4. The pre-loaded spec contents are inlined in the planner preamble. Detect explicit dependency language ("depends on Phase N", "after Phase N", "requires Phase N") and encode it via the optional \`dependsOn: string[]\` field on each chunk (list other chunk \`id\`s); default to no \`dependsOn\` (parallel) when the spec doesn't state a dependency. If a chunk would honestly be tagged \`estimatedComplexity: "high"\`, subdivide it into 2–4 sibling chunks (sharing the same \`dependsOn\`) before emitting the JSON. Cap subdivision at one pass.`,
    },
    codingConfig: {
