...
function formatContent(content) {
    ...
    segments.forEach((seg, i) => {
        if (seg.startsWith('```') && seg.endsWith('```')) {
            ...
            parts.push(<pre key={i} className="my-2 overflow-x-auto rounded-lg bg-zinc-900 p-3 text-xs text-zinc-300">
          <code>{code}</code>
        </pre>);
        }
        else if (seg.startsWith('`') && seg.endsWith('`')) {
            parts.push(<code key={i} className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs text-blue-400">
          {seg.slice(1, -1)}
        </code>);
        }
        else if (seg.startsWith('**') && seg.endsWith('**')) {
            parts.push(<strong key={i} className="font-semibold">
          {seg.slice(2, -2)}
        </strong>);
        }
        else {
...
export function MessageBubble({ message }) {
    ...
    if (type === 'dag-dispatched' && dagId) {
        ...
        <div className="max-w-[85%]">
          {dag ? (<InlineDAGCard dag={dag}/>) : (<div className="rounded-2xl bg-zinc-800 px-4 py-3 text-sm text-zinc-100">
    ...
    if (type === 'dag-confirmation' && dagId && pendingConfirmation?.dagId === dagId) {
        return (<div className="my-3 flex justify-start">
        <div className="max-w-[85%]">
          <DAGConfirmationCard confirmation={pendingConfirmation} onRespond={respondToConfirmation}/>
        </div>
      </div>);
    }
    ...
    if (role === 'system') {
        return (<div className="my-3 flex justify-center">
    ...
    return (<div className={`my-3 flex ${isUser ? 'justify-end' : 'justify-start'}`}>
