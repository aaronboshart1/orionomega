...
function NodeRow({ node }) {
    ...
    return (<div>
      <button onClick={() => hasOutput && setExpanded(!expanded)} className={`flex w-full items-center gap-2 rounded px-2 py-1 text-xs transition-colors ${hasOutput ? 'cursor-pointer hover:bg-zinc-700/50' : 'cursor-default'}`}>
        ...
        {node.status === 'running' && node.progress !== undefined && (<span className="text-[10px] text-blue-400">{node.progress}%</span>)}
        {hasOutput && (expanded
            ? <ChevronDown size={10} className="text-zinc-500"/>
            : <ChevronRight size={10} className="text-zinc-500"/>)}
      </button>
      {expanded && node.output && (<div className="ml-6 mt-1 mb-1 rounded bg-zinc-900 px-3 py-2 text-[11px] text-zinc-400">
          {node.output}
        </div>)}
    </div>);
}
export function InlineDAGCard({ dag }) {
    const [expanded, setExpanded] = useState(false);
    ...
    const progressPct = dag.totalCount > 0
        ...
        : 0;
    return (<div className={`rounded-xl border px-4 py-3 transition-colors ${isActive
      ...
      <div className="flex items-center gap-2">
        ...
        {dag.nodes.length > 0 && (<button onClick={() => setExpanded(!expanded)} className="rounded p-0.5 text-zinc-500 hover:bg-zinc-700 hover:text-zinc-300">
            {expanded
                ? <ChevronDown size={14}/>
      ...
      {/* Progress bar */}
      {isActive && (<div className="mt-2 h-1 w-full overflow-hidden rounded-full bg-zinc-700">
      ...
      {isActive && dag.elapsed > 0 && (<div className="mt-1 text-right text-[10px] text-zinc-600">
          {dag.elapsed < 60 ? `${Math.round(dag.elapsed)}s` : `${Math.floor(dag.elapsed / 60)}m ${Math.round(dag.elapsed % 60)}s`}
        </div>)}

      {/* Expanded node list */}
      {expanded && dag.nodes.length > 0 && (<div className="mt-2 space-y-0.5 border-t border-zinc-700/50 pt-2">
          {dag.nodes.map((node) => (<NodeRow key={node.id} node={node}/>))}
        </div>)}

      {/* Error message */}
      {isError && dag.error && (<p className="mt-2 text-xs text-red-400">{dag.error}</p>)}
    </div>);
}
//# sourceMappingURL=InlineDAGCard.js.map