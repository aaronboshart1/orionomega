...
export function ChatPane() {
    ...
    const hasActiveDAGs = Object.values(inlineDAGs).some((d) => d.status === 'dispatched' || d.status === 'running');
    ...
    return (<div className="flex h-full flex-col bg-[var(--background)]">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {messages.length === 0 && (<div className="flex h-full flex-col items-center justify-center text-zinc-600">
        ...
        {activePlan && showAdvancedPlan && (<div className="my-4">
            <PlanCard plan={activePlan} onRespond={respondToPlan}/>
          </div>)}

        {/* Subtle plan notification when plan arrives but advanced view is hidden */}
        {activePlan && !showAdvancedPlan && (<div className="my-3 flex justify-start">
            <button onClick={() => setShowAdvancedPlan(true)} className="flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-800/60 px-4 py-2 text-xs text-zinc-400 transition-colors hover:border-zinc-600 hover:text-zinc-300">
              <Settings2 size={12}/>
