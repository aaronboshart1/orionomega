...
export function WorkflowSummary() {
    ...
    const progressPct = stats.total > 0 ? Math.round(((stats.done + stats.error) / stats.total) * 100) : 0;
    return (<div className="flex h-full items-center gap-6 px-6">
      ...
      <div className="flex items-center gap-3 text-xs">
        {stats.running > 0 && (<span className="text-blue-400">
            {stats.running} running
          </span>)}
        {stats.done > 0 && (<span className="text-green-400">
            {stats.done} done
          </span>)}
        {stats.pending > 0 && (<span className="text-zinc-500">
            {stats.pending} pending
          </span>)}
        {stats.error > 0 && (<span className="text-red-400">
            {stats.error} error
          </span>)}
      </div>
...
//# sourceMappingURL=WorkflowSummary.js.map